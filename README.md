...
Użyte polecenia:
* `mkdir -p ~/lab12/web1_logs ~/lab12/web2_logs ~/lab12/web3_logs` – parametr `-p` pozwala na utworzenie całej ścieżki katalogów za jednym razem (tworzy główny katalog `lab12` oraz podkatalogi na logi dla każdego z trzech serwerów).
* `echo "<h1>Laboratorium 12</h1><p>Jakub Fus</p>" > ~/lab12/index.html` – polecenie to generuje prostą stronę HTML z moimi danymi i za pomocą operatora `>` zapisuje ją do pliku `index.html` w nowo utworzonym katalogu.
* `ls lab12` – weryfikacja, czy pliki i foldery zostały poprawnie utworzone.
...
* `--name web1` – nadaje kontenerowi przyjazną nazwę.
...
* `--mount type=bind,source=$HOME/lab12/web1_logs,target=/var/log/nginx` – mapuje katalog logów. Zawartość generowana przez serwer w ścieżce `/var/log/nginx` jest automatycznie zapisywana na dysku twardym laptopa w dedykowanym katalogu.
...
* `curl localhost:8081` (oraz dla portów 8082 i 8083) – narzędzie to wysłało żądanie pobrania strony ze wskazanych portów. W odpowiedzi otrzymałem zdefiniowany wcześniej kod HTML (`<h1>Laboratorium 12</h1><p>Jakub Fus</p>`). 
...
Użyte polecenie:
* `cat ~/lab12/web1_logs/access.log` (oraz dla pozostałych katalogów `web2_logs` i `web3_logs`) – polecenie `cat` służy do wypisania zawartości pliku tekstowego w terminalu.

Wyniki w terminalu pokazały prawidłowe wpisy o ruchu HTTP (zawierające znacznik czasowy oraz informację o programie klienckim `curl/8.5.0`). Dowodzi to, że podłączenie wolumenów zadziałało prawidłowo i Nginx zapisuje swoje logi bezpośrednio w systemie plików używanego laptopa.
